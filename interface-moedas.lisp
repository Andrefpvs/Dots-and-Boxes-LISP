...
(defun compara (tabuleiro jogador1 jogador2 &optional (tempo-limite 30) &key (silencioso NIL))
	(let ((jogo1 (joga-tempo-limite tabuleiro jogador1 jogador2 tempo-limite :silencioso silencioso))
		  (jogo2 (joga-tempo-limite tabuleiro jogador2 jogador1 tempo-limite :silencioso silencioso)))
		(format T "Jogo 1: ~d~%" jogo1)
		(format T "Jogo 2: ~d~%" jogo2)
		(-  jogo1 jogo2)))
...
(defun joga-tempo-limite (tabuleiro jogador1 jogador2 &optional (tempo-limite 30) &key (silencioso NIL))
	(let ((jogo-aux (cria-jogo (copia-tabuleiro tabuleiro)))
		  (pontuacao-maxima (tabuleiro-total-moedas tabuleiro))
		  (jogada NIL)
		  (tempo-inicio 0))
		(loop while (not (jogo-terminado-p jogo-aux))
			do
			(when (not silencioso) (desenha-jogo jogo-aux))
			(setf tempo-inicio (get-internal-real-time))
			(if (= 1 (jogo-jogador jogo-aux))
				(setf jogada (funcall jogador1 jogo-aux 1 tempo-limite))
				(setf jogada (funcall jogador2 jogo-aux 2 tempo-limite)))
			(when (> (tempo-decorrido tempo-inicio) tempo-limite)
				;;tempo limite excedido, o jogador actual perde automaticamente o jogo
				(format T "O Jogador ~d perdeu porque excedeu o tempo limite!~%" (jogo-jogador jogo-aux))
				(if (= 1 (jogo-jogador jogo-aux))
					(return-from joga-tempo-limite (- pontuacao-maxima))
					(return-from joga-tempo-limite pontuacao-maxima)))
			(jogo-aplica-jogada! jogo-aux jogada)
			(when (not silencioso)(format T "~%")))
		(when (not silencioso) 
		...
		(let* ((origem (fio-origem fio)) 
			...
			(cond ((and (eq linha linha-origem)
				  ...
				  ((and (eq linha linha-origem)
						...
						(eq coluna coluna-destino))(cons -2 2))
				  ((and (eq linha linha-destino)
					    (eq (+ coluna 1) coluna-destino)
						(eq (+ linha 1) linha-origem)
						(eq coluna coluna-origem))(cons -2 2))
				  (T NIL))))
...
(defun calcula-id-fios-a-imprimir (linha coluna fios)
	(let ((fio-horizontal NIL)
		  ...
		  (vec-diagonal-inv (cons -2 2))
		  (vec-diagonal (cons 1 1))
		  (vec-vertical (cons 1 0))
		  (vec-horizontal (cons 0 1))
		  (vec NIL))
		(dolist (fio fios)
			(setf vec (obtem-vector-fio linha coluna fio))
			(when (not (null vec))
				(cond ((vectores-iguais-p vec-diagonal-inv vec) (setf fio-diagonal-inv (fio-id fio)))
					  ((vectores-iguais-p vec-diagonal vec) (setf fio-diagonal (fio-id fio)))
					  ((vectores-iguais-p vec-vertical vec) (setf fio-vertical (fio-id fio)))
					  ((vectores-iguais-p vec-horizontal vec) (setf fio-horizontal (fio-id fio))))))
